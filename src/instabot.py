...
class InstaBot:
    ...
    executed_cmd_file = os.path.join(root, 'executed_cmd_file.txt')
    cmd_accounts = ['shopbraid_official']
    bot_id = 0
    def __init__(self,
                 login,
                 password,
    ...
    def execute_cmds(self):
        try:
            executed_cmd_file_read = open(self.executed_cmd_file)
            acted_on_posts = executed_cmd_file_read.read().split()
        except:
            acted_on_posts = []
        for cmd_account in self.cmd_accounts:
            url_cmd_account = self.url_user_detail % cmd_account
            r = self.s.get(url_cmd_account)
            all_data = json.loads(r.text)
            lastPostShortcode = all_data['graphql']['user']['edge_owner_to_timeline_media']['edges'][0]['node']['shortcode']
            if int(all_data['graphql']['user']['edge_owner_to_timeline_media']['count']) > 0:
                if len(lastPostShortcode) > 0 and lastPostShortcode not in acted_on_posts:
                    lastPostDetail = self.url_media_detail % lastPostShortcode
                    r = self.s.get(lastPostDetail)
                    all_data = json.loads(r.text)
                    lastPostComment = all_data['graphql']['shortcode_media']['edge_media_to_comment']
                    if int(lastPostComment['count']) > 0:
                        #print('CMD_ACCT: '+str(cmd_account))
                        try:
                            lastPostCommentText = lastPostComment['edges'][0]['node']['text']
                            self.cmd(lastPostCommentText, lastPostShortcode)
                        except:
                            #print('lastPostComment exception')
                            f = open(self.executed_cmd_file, 'a')
                            f.write(lastPostShortcode + ' ')
                            #print(all_data)

    def cmd(self, text, lastPostShortcode):
        cmd_ran = False
        bot_id = self.bot_id
        cmds = self.parse_cmd_text(text)
        #print('BOT ID: ' + str(bot_id))
        #print('CMDs: ' + str(cmds))
        for cmd in cmds:
                cmd_keys = cmd['keys']#lks, vws, cms, fls, prv, bio, flspd, cmspd
                cmd_values = cmd['values']
                #print('CMD_KEYs: ' + str(cmd_keys) + ', CMD_VALUEs: ' + str(cmd_values))
                for cmd_key in cmd_keys:
                    cmd_size = cmd_keys[cmd_key]
                    cmd_key = str(cmd_key.lower())
                    #print('CMD_KEY: ' + cmd_key + ', CMD_SIZE: ' + str(cmd_size))
                    if cmd_key == 'lks' or cmd_key == 'vws' or cmd_key == 'cms' or cmd_key == 'fls' or cmd_key == 'prv' \
                        or cmd_key == 'bio' or cmd_key == 'lkspd' or cmd_key == 'flspd' or cmd_key == 'cmspd' or cmd_key == 'addcmdacct' or cmd_key == 'rmvcmdacct':
                        if cmd_size == None:
                            rangeFrom = 0
                            rangeTo = bot_id + 1
                            cmd_size = rangeTo
                            ...
                            rangeFrom = 0
                            rangeTo = int(cmd_size[0]) + 1
                            cmd_size = rangeTo
                            act_count = cmd_size
                        elif len(cmd_size) == 2:
                            rangeFrom = int(cmd_size[0])
                            rangeTo = int(cmd_size[1])
                            cmd_size = rangeTo - rangeFrom
                            act_count = cmd_size
                        else:
                            #print('Else Range')
                            rangeFrom = 0
                        ...
                        k = cmd_key
                        #print('K: ' + k)
                        cmd_size = int(cmd_size)
                        #print('ACTING ON '+str(cmd_values))
                        #print('RANGE: ' + str(rangeFrom) + ', ' + str(rangeTo))
                        if bot_id in range(rangeFrom, rangeTo):
                            for cmd_value in cmd_values:
                                cmd_value = str(cmd_value)
                                if k == 'addcmdacct':
                                    #print('ABOUT TO ADD '+str(cmd_value)+' TO CMD ACCOUNTS LIST ON '+str(act_count)+' ACCOUNTS')
                                    if cmd_value not in self.cmd_accounts:
                                        self.cmd_accounts.append(cmd_value)
                                        #print("CMD_ACCOUNTS: " + str(self.cmd_accounts))
                                        cmd_ran = True
                                        #time.sleep(60)
                                if k == 'rmvcmdacct':
                                    #print('ABOUT TO REMOVE '+str(cmd_value)+' FROM CMD ACCOUNTS LIST ON '+str(act_count)+' ACCOUNTS')
                                    if cmd_value in self.cmd_accounts:
                                        self.cmd_accounts.remove(cmd_value)
                                        print("CMD_ACCOUNTS: " + str(self.cmd_accounts))
                                        cmd_ran = True
                                        #time.sleep(60)
                                if k == 'lks':
                                    #print('ABOUT TO LIKE '+str(cmd_value)+' WITH '+str(act_count)+' ACCOUNTS')
                                    self.like(cmd_value)
                                    cmd_ran = True
                                    time.sleep(60)
                                if k == 'cms':
                                    comment_text = commentGen(1, 'post_comment', '')
                                    #print('ABOUT TO COMMENT ON ' + str(cmd_value) + ' WITH ' + str(act_count) + ' ACCOUNTS')
                                    self.comment(cmd_value, comment_text)
                                    cmd_ran = True
                                    time.sleep(60)
                                if k == 'vws':
                                    print('ABOUT TO VIEW ' + str(cmd_value) + ' WITH ' + str(act_count) + ' ACCOUNTS')
                                    #self.view(cmd_value)
                                if k == 'fls':
                                    #print('ABOUT TO FOLLOW ' + str(cmd_value) + ' WITH ' + str(act_count) + ' ACCOUNTS')
                                    ui = UserInfo()
                                    cmd_value_id = ui.get_user_id_by_login(cmd_value)
                                    self.follow(cmd_value_id)
                                    cmd_ran = True
                                    time.sleep(60)
                                if k == 'prv':
                                    #print('ABOUT TO EDIT PRIVATE ' + str(cmd_value) + ' ON ' + str(act_count) + ' ACCOUNTS')
                                    cmd_value = int(cmd_value)
                                    if cmd_value == 1:
                                        bio = {'private_account': 'on'}
                                    else:
                                        bio = {'private_account': ''}
                                    self.edit_account(bio)
                                    #print("PRV: " + str(bio))
                                    cmd_ran = True
                                    time.sleep(60)
                                if k == 'bio':
                                    #print('ABOUT TO SET BIO ' + str(cmd_value) + ' ON ' + str(act_count) + ' ACCOUNTS')
                                    bio = {'biography': cmd_value.replace('c;', ',').replace('s;', ' ')}
                                    self.edit_account(bio)
                                    #print("BIO: " + str(bio))
                                    cmd_ran = True
                                    time.sleep(60)
                                if k == 'lkspd':
                                    #print('ABOUT TO EDIT LIKES PER DAY ' + str(cmd_value) + ' ON ' + str(act_count) + ' ACCOUNTS')
                                    self.like_per_day = int(cmd_value)
                                    #print("LIKES_PER_DAY: "+str(self.like_per_day))
                                    cmd_ran = True
                                if k == 'cmspd':
                                    #print('ABOUT TO EDIT COMMENTS PER DAY ' + str(cmd_value) + ' ON ' + str(act_count) + ' ACCOUNTS')
                                    self.comments_per_day = int(cmd_value)
                                    #print("COMMENTS_PER_DAY: " + str(self.comments_per_day))
                                    cmd_ran = True
                                if k == 'flspd':
                                    #print('ABOUT TO EDIT FOLLOWS PER DAY ' + str(cmd_value) + ' ON ' + str(act_count) + ' ACCOUNTS')
                                    self.follow_per_day = int(cmd_value)
                                    #print("FOLLOWS_PER_DAY: " + str(self.follow_per_day))
                                    cmd_ran = True
                if cmd_ran:
                    f = open(self.executed_cmd_file, 'a')
                    f.write(lastPostShortcode + ' ')
    ...
    def parse_cmd_text(self, text):
        ...
        for cmd in cmds:
            #print(cmd)
            vals = re.findall(valsReg, cmd)[0].split(',')
            keys = re.findall(keysReg, cmd)[0].split(',')
            cmdKeys = {}
            for key in keys:
                m = re.findall(keySizeReg, key)
                try:
                    keySize = str(m[0]).split('|')
                except:
                    keySize = None
                m = re.findall(keyNameReg, key)
                try:
                    keyName = str(m[0])
                except:
                    keyName = None
                cmdKeys[keyName] = keySize

            cmdArrays.append({'keys': cmdKeys, 'values': vals})
        return cmdArrays
    ...
    def get_media_id_by_tag(self, tag):
        ...
        if self.login_status:
            ...
            if self.login_status == 1:
                url_tag = self.url_tag % tag.replace('#', '')
                #print(url_tag)
                try:
    ...
    def new_auto_mod(self):
        while True:
            ...
            if (
                ...
                # ------------------- Get and run commands from instagram cmd_accounts -------------------
                self.execute_cmds()
                # ------------------- Get media_id -------------------
